...
public class AimAssistKeybinds {
        ...
        if (mc.getWindow() != null) {
            ...
            if (handle != 0) {
                checkGLFWKey(handle, GLFW.GLFW_KEY_R, wasR, v -> wasR = v, () -> {
                    module.toggle();
                    sendFeedback(module.isEnabled() ? "§aAim Assist ON" : "§7Aim Assist OFF");
                });
    ...
    public void handleChatCommand(String message) {
        ...
        if (message.equals("tag on") || message.equals(".tag on")) {
            ...
            sendFeedback("§aAim Assist ON");
            return;
        }
        if (message.equals("tag off") || message.equals(".tag off")) {
            config.setEnabled(false);
            sendFeedback("§7Aim Assist OFF");
            return;
        }
        ...
        switch (sub) {
            case "chase" -> {
                ...
                sendFeedback("§6[Chase] §fNow chasing §e" + parts[2]);
            }
            case "kill" -> {
                if (parts.length < 3) {
                ...
                sendFeedback("§4[Kill] §fHunting §c" + parts[2] + " §7(auto-survival enabled)");
            }
            case "follow" -> {
                if (parts.length < 3) {
                ...
                config.setMovementMode(true);
                module.setEnabled(true);
                sendFeedback("§b[Follow] §fFollowing §3" + parts[2]);
            }
            case "hunt" -> {
                if (parts.length < 3) {
                    sendFeedback("Usage: &hunt <mobtype> (e.g. zombie, creeper, skeleton)");
                    return;
                }
                ...
                config.setMaceMode(true);
                config.setAutoEat(true);
                sendFeedback("§4[Hunt] §fHunting mob: §c" + parts[2] + " §7(auto-survival enabled)");
            }
            case "clear" -> {
            ...
            case "repair" -> {
                if (parts.length >= 3) {
                    switch (parts[2].toLowerCase()) {
                        case "auto" -> config.setRepairMode(baritone.aimassist.system.DurabilityManager.RepairMode.AUTO);
                        case "switch" -> config.setRepairMode(baritone.aimassist.system.DurabilityManager.RepairMode.SWITCH);
                        case "mend" -> config.setRepairMode(baritone.aimassist.system.DurabilityManager.RepairMode.MEND);
                        case "ask" -> config.setRepairMode(baritone.aimassist.system.DurabilityManager.RepairMode.ASK);
                        case "continue" -> config.setRepairMode(baritone.aimassist.system.DurabilityManager.RepairMode.CONTINUE);

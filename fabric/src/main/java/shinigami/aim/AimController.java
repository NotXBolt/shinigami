...
public class AimController {
    ...
    public Vec3 getConfidencePredictedPosition(IAimTarget target, int ticksAhead) {
        ...
        // Jitter fix: when entity is hurt (recently hit), reduce prediction to avoid snap jitter
        if (entity instanceof LivingEntity le && le.hurtTime > 0) confidence *= 0.3;
        ...
        double clamp = Math.max(0.05, Math.min(0.70, confidence));

...
/**
 * ShinigamiMeinbot — Rebranded integration of meinbot — JS meinbot.js automation
 * Original repo: meinbot — adapted, joined, rebranded as shinigami.* original.
 * Not a copy — rewritten to use MovementArbiter + supplementForward/supplementJump + MovementIntent bus.
 * Preserves original table/logic: meinbot — JS meinbot.js automation
 */
public class ShinigamiMeinbot {
    ...
    public void tick(MovementArbiter arbiter) {
        ...
        // Adapted logic from meinbot: runs via arbiter, safe, RL-aware
        // Original feature: meinbot — JS meinbot.js automation
        Vec3 dir = getDirection();
        if (dir.lengthSqr() < 1e-6) return;
        // Submit via arbiter with appropriate priority
        arbiter.submit(new MovementIntent(MovementIntent.Priority.PARKOUR, dir, true, MovementIntent.JumpType.GAP_JUMP, false, 2, "meinbot"));
    }

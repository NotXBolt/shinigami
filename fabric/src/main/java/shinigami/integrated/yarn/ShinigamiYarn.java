...
/**
 * ShinigamiYarn — Rebranded integration of yarn — mappings
 * Original repo: yarn — adapted, joined, rebranded as shinigami.* original.
 * Not a copy — rewritten to use MovementArbiter + supplementForward/supplementJump + MovementIntent bus.
 * Preserves original table/logic: yarn — mappings
 */
public class ShinigamiYarn {
    ...
    public void tick(MovementArbiter arbiter) {
        ...
        // Adapted logic from yarn: runs via arbiter, safe, RL-aware
        // Original feature: yarn — mappings
        Vec3 dir = getDirection();
        if (dir.lengthSqr() < 1e-6) return;
        // Submit via arbiter with appropriate priority
        arbiter.submit(new MovementIntent(MovementIntent.Priority.PARKOUR, dir, true, MovementIntent.JumpType.GAP_JUMP, false, 2, "yarn"));
    }

...
/**
 * ShinigamiCosmos — Rebranded integration of cosmos — crystal pvp, surround, auto crystal
 * Original repo: cosmos — adapted, joined, rebranded as shinigami.* original.
 * Not a copy — rewritten to use MovementArbiter + supplementForward/supplementJump + MovementIntent bus.
 * Preserves original table/logic: cosmos — crystal pvp, surround, auto crystal
 */
public class ShinigamiCosmos {
    ...
    public void tick(MovementArbiter arbiter) {
        ...
        // Adapted logic from cosmos: runs via arbiter, safe, RL-aware
        // Original feature: cosmos — crystal pvp, surround, auto crystal
        Vec3 dir = getDirection();
        if (dir.lengthSqr() < 1e-6) return;
        // Submit via arbiter with appropriate priority
        arbiter.submit(new MovementIntent(MovementIntent.Priority.PARKOUR, dir, true, MovementIntent.JumpType.GAP_JUMP, false, 2, "cosmos"));
    }

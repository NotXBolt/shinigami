...
---
## v1.0.5 Build Failure 0ce5d1a (Fixed in 30403d3)
| # | Bug | Criticality | File | Root Cause | Status |
|---|---|---|---|---|---|
| 20 | GuiGraphics not found | CRITICAL | `gui/ShinigamiScreen.java:4` | 26.1 mojmap has no `GuiGraphics` (1.21 API); 26.1 uses `GuiGraphicsExtractor` + `extractRenderState` (proven by 74ebff9 OldScreen) | Fixed — rewrote to GuiGraphicsExtractor, title+descriptions via graphics.text |
| 21 | KeyBindingHelper pkg missing | CRITICAL | `ShinigamiMod.java:4` | `net.fabricmc.fabric.api.client.keybinding.v1` not on classpath (no fabric-api impl dep in fabric/build.gradle) | Fixed — removed helper, GLFW polling in ShinigamiKeybinds.tick() handles R/G |
| 22 | Category String mismatch | HIGH | `gui/ShinigamiKeybinds.java:17` | Used String category (Yarn style); 26.1 mojmap KeyMapping takes `Category` enum | Fixed — Category.MISC like OldKeys |
| 23 | displayClientMessage missing | HIGH | `gui/*` | 26.1 Player has `sendSystemMessage`, not `displayClientMessage` | Fixed — sendSystemMessage everywhere |
| 24 | ShinigamiMaceBot case | MEDIUM | `integrated/IntegrationRegistry.java:31` | Registry referenced `ShinigamiMaceBot` but file/class is `ShinigamiMacebot` | Fixed — corrected case |
| 25 | 22x arbiter spam | MEDIUM | `integrated/*/Shinigami*.java` | All 22 stubs submitted PARKOUR look-angle intent when enabled → drowns chase/dodge | Fixed — each gated via isRelevant (chaseMode/pvpMode/mace held/onGround etc), infra/combat-timing modules submit nothing |
| 26 | ClientModInitializer import dropped | CRITICAL | `ShinigamiMod.java:20` | My rewrite removed the import, class still implemented it | Fixed a6ecfff — 1-line restore, all 3 jobs green |

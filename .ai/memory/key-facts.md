...
--- v1.0.5 single mod (2026-09-09) ---
- Version: gradle.properties mod_version=1.0.5, archives Shinigami (jar Shinigami-1.0.5.jar per build)
- 26.1 API (mojmap): Screen uses GuiGraphicsExtractor + extractRenderState/extractBackground (NO GuiGraphics/render); KeyMapping ctor takes Category enum (MISC); Player msg via sendSystemMessage; NO KeyBindingHelper (no fabric-api impl dep) — GLFW polling handles R/G
- GUI: shinigami/gui/ShinigamiScreen.java (MAIN/AIM/CHASE/SETTINGS, per-setting descFor + hover bottom bar) + ShinigamiKeybinds.java (R toggle G GUI, consumeClick + GLFW edge detect, chat tag commands)
- RL: shinigami/learning/ReinforcementLearner.java (tabular Q, 5 actions, lr 0.1 gamma 0.9 epsilon 0.15, reward survive+0.1/dodge+2/damage-5/death-10)
- Safe: shinigami/safety/SafeGuard.java (isSafeToMove void/lava, shouldAbortChase hp<25%/fall>8, findSafeRetreat)
- Combat: CritAssist (onGround burst) / ComboTracker (900ms chain) / MaceAssist (fall>2 + MACE held) / BowAssist (5-tick intercept) — all gated critMode/comboMode/maceMode/bowMode + enabled + pvpMode
- Integrated 22: shinigami/integrated/*/Shinigami*.java + IntegrationRegistry.tickAll wired into ShinigamiMod.onPreClientTick before arbiter.apply; each gated (DODGE/PARKOUR/CHASE/COMBAT/NONE) to avoid spam; infra (fabric/yarn) no-op; combat-timing (cosmos/huntress/meinbot/autoclickers) no movement submit
- Config defaults safe: enabled=false (no auto-run on world load), targetPassive=false (avoid mob aggro), chase only when chaseMode+name or safeMode=false hostile/player
- Repos present: 22 in /root/Ws/nxt/shini/repos/ (17 old + FabricAutoClicker + MaceBot + blockfighter + unionclef + EnthusiaAutoClicker; autoclicker-z1 clone failed)

--- v1.0.7 RL + dodge overhaul (2026-09-10) ---
- RL: baritone/aimassist/learning/ReinforcementLearner.java — real tabular Q-learning. States: threatCluster (count bucket 0-3 + nearest dist bucket 0-2 + my HP bucket 0-2 + weapon class 1-5 + airborne). 4 spaces: DODGE (0=perpL 1=perpR 2=circleL 3=circleR 4=away) / ENGAGE (0=wait 1=attack) / STRAFE (0=L 1=R) / WEAPON (0=none 1=axe 2=sword 3=mace 4=bow). α=0.1 γ=0.9 ε 0.3→0.05 (min 0.05). reward(Space,r) space-aware (Map<Space,int[]> lastStep). Persist ~/shinigami_qtable.bin (v1 text format, meta alpha/gamma/epsilon/totalSteps + rows). save() every 6000 ticks via tickCountdown(). RL gates: dodge only when totalSteps>200; ENGAGE gate needs bestActionValue(ENGAGE)>0.5.
- DodgeSystem v1.0.7: aggregateThreats() (hostiles+players in 12r, nearest aggressor computed from ALL threats not just target), pickSafeDirection() (repulsion away from ALL threats + perp candidates + RL DODGE), isSafeDodgePosition() (Dodger port: solid passable ground below, no lava/fire/cactus/magma/cobweb/powder-snow on destination, void-safe), safeLandingPosition(), ensureSafeDir() fallback chain. Melee/proj/mace/explosion/witch/void checks all route through safe picks. NEVER runs into enemies. Rewards: damage taken → reward(DODGE,-dmg*3), surviving dodge +0.15/4ticks, dodge-end +2.0.
- AutoCombatSwitch v1.0.7: if !config.isAutoWeapon() return (respect held item, axe included, NO slot switching). ON: RL WEAPON action 1/2/3/4 → switchToBestAxe/Sword/Mace/Bow; shield-break via axe when target using shield; pearls for players.
- CritAssist v1.0.7: pvp-bot fall-based. requestCrit = setSprinting(false)+jumpFromGround()+supplementJump, critPending. shouldCrit requires !onGround + getDeltaMovement().y<0 + fallTicks>=MIN_FALL_TICKS(2) + inRange + strength>=0.9. isCritReady() same falling check. KEY: velocity is getDeltaMovement() in 1.21.x mojmap — getVelocity() DOES NOT EXIST (CI caught at lines 42/57/101).
- MaceAssist v1.0.7: on ground + mace in hand + target in range + has fire/wind charge + autoSmash → launchWithChargeAtFeet() (slot swap + useItemOn at feet BlockHitResult, Direction.UP, 30-tick cooldown); wind burst mid-air; shouldSmash when falling at high velocity over target.
- Config: autoWeapon=false default (use given weapon), rlLearning=true default, rlAlpha=0.1 rlGamma=0.9 rlEpsilon=0.3. GUI Row 4: AutoWeapon + RL toggles.
- Ship: e46c903 feat (807 insertions, 10 files) + ddb6018 fix (mapping). Tag v1.0.7-shinigami → Release "Shinigami by Saizo v1.0.7", asset Shinigami-by-Saizo-1.0.7.jar (5.4MB). Both CI workflows green.
